<template>
  <div class="table">
    ...
    <div class="container">
      <el-container>
        <!-- <el-aside width="200px" style="height:800px;overflow:auto">
          <el-tree
            :data="deptTreeData"
            :props="defaultProps"
            accordion
        ...
        <el-container>
          ...
          <el-main>
            ...
            >
              ...
              <el-table-column prop="levelName" align="center" label="用户级别"></el-table-column>
    ...
    <el-dialog title="编辑" :visible.sync="editVisible" width="50%">
      <el-form ref="form" :model="form" label-width="100px">
        ...
        <el-form-item label="用户级别">
          <el-select v-model="form.level" placeholder="用户级别">
            <el-option
              label="预存款用户"
              value="1"
            ></el-option>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      SysUserApi.getData(this.req).then(
        res => {
          this.loading = false;
          if (res.data.error === false) {
            ...
            this.tableData.forEach(item => {
              if(item.level==1){
                // 1:预存款用户,0:非预存款用户，2：管理员
                item.levelName="预存款用户"
              }else if(item.level==0){
                item.levelName="非预存款用户"
    ...
    handleEdit(index, row) {
      ...
      SysUserApi.info({ userId: row.id }).then(
        res => {
          this.loading = false;
          if (res.data.error === false) {
    ...
    saveEdit() {
      ...
      this.loading = true;
      SysUserApi.save(this.form).then(
        res => {
          this.loading = false;
          if (res.data.error === false) {
    ...
    // 确定删除
    deleteRow() {
      SysUserApi.batchDelete(this.ids).then(
        res => {
          if (res.data.error === false) {
            this.$message.success(res.data.msg);
            this.reload();
    ...
    handleResetPassword(id) {
      ...
      this.$confirm("是否确认将密码重置为：123456？", "提示", {
        ...
        SysUserApi.resetPassword(ids).then(
          res => {
            if (res.data.error === false) {
              this.$message.success(res.data.msg);
              this.reload();
    ...
    changeStatus(id, flag) {
      SysUserApi.changeStatus(id, !flag ? 0 : 1).then(
        res => {
          if (res.data.error === false) {
