  ...
  <div class="table">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-lx-people"></i> 部门管理
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
...
<!-- <el-table :data="treeDataTable">
  <el-table-column v-for="(item,index) in columns" :key="index" :label="item.text" :prop="item.value">   
  </el-table-column>
  <el-table-column type="expand">
       <template slot-scope="scope">
         <el-table :data="scope.row.children"></el-table>
        </template>
     </el-table-column>
</el-table> -->
...
export default {
  ...
  methods: {
    ...
    getTreeData() {
      DeptApi.getTreeData(this.req).then(
        res => {
          this.loading = false;
          if (res.data.error === false) {
            this.treeData = res.data.data;
            this.treeDataTable= res.data.data[0].children
          } else {
            this.$message.error(res.data.msg);
          }
        },
    ...
    handleEdit(index, row) {
      DeptApi.info({ deptId: row.id }).then(
        res => {
          if (res.data.error === false) {
    ...
    // 确定删除
    deleteRow() {
      DeptApi.batchDelete(this.ids).then(
        res => {
          if (res.data.error === false) {
            this.$message({
              message: "操作成功",
