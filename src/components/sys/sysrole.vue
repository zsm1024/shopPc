  ...
  <div class="table">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-lx-people"></i> 角色管理
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
        ...
        <el-table-column label="操作" width="180" align="center">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              @click="handleConfigPerms(scope.$index, scope.row)"
            >权限配置</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    saveMuenPerms() {
      this.checkMenuData = [];
      this.checkMenuData = this.checkMenuData.concat(
        this.$refs.treeMenu.getCheckedKeys()
      );
      this.checkMenuData = this.checkMenuData.concat(
        this.$refs.treeMenu.getHalfCheckedKeys()
      );
      RoleApi.saveMuenPerms({
        id: this.roleId,
        menuIds: this.checkMenuData
      }).then(
        res => {
    ...
    getCheckMenuData(roleId) {
      RoleApi.getCheckMenuData({ roleId: roleId }).then(
        res => {
          if (res.data.error === false) {
            this.checkMenuData = res.data.data;
          } else {
            this.$message.error(res.data.msg);
          }
        },
