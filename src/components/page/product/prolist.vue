  ...
  <div class="table">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-lx-people"></i>商品分类管理
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      <div class="handle-box">
        <el-input style="width: 130px" v-model="req.name" placeholder="请输入商品名称"></el-input>
        <span class="bttn1">
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      RoleApi.categorylist(this.req).then(
        res => {
          this.loading = false;
          if (!res.data.error) {
    ...
    // 确定删除
    deleteRow() {
      RoleApi.categorydelete(this.ids).then(
        res => {
          if (!res.data.error) {
            this.$message({
              message: "删除成功",
