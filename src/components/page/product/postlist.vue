  ...
  <div class="table">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-lx-people"></i>快递管理
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      <div class="handle-box">
        <el-input style="width: 130px" v-model="req.province" placeholder="请输入省份名称"></el-input>
        <span class="bttn1">
    ...
    >
      ...
      >
        <el-form-item label="省份" prop="province">
          <el-input v-model.trim="role.province"></el-input>
        </el-form-item>
        <el-form-item label="起步价（元）" prop="startingPrice">
          <el-input v-model.trim="role.startingPrice"></el-input>
        </el-form-item>
        <el-form-item label="起步重量（kg）" prop="startingWeight">
          <el-input v-model.trim="role.startingWeight"></el-input>
        </el-form-item>
        <el-form-item label="阶梯重量(kg)" prop="stepWeight">
          <el-input v-model.trim="role.stepWeight"></el-input>
        </el-form-item>
        <el-form-item label="阶梯价格(元)" prop="stepPrice">
          <el-input v-model.trim="role.stepPrice"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      tableColum: [
        ...
        { title: "起步价（元）", prop: "startingPrice" },
        { title: "起步重量（kg）", prop: "startingWeight" },
        { title: "阶梯重量(元)", prop: "stepWeight" },
        { title: "阶梯价格(元)", prop: "stepPrice" }
      ],
  ...
  methods: {
    ...
    getData() {
      ...
      RoleApi.expressDeliverylist(this.req).then(
        res => {
          this.loading = false;
          if (!res.data.error) {
    ...
    saveEdit(role) {
      ...
      this.$refs[role].validate(valid => {
        if (valid) {
          this.loading = true;
          RoleApi.expressDelivery(this.role).then(
            res => {
              this.loading = false;
              if (!res.data.error) {
                this.editVisible = false;
    ...
    // 确定删除
    deleteRow() {
      RoleApi.expressDeliverydelete(this.ids).then(
        res => {
          if (!res.data.error) {
            this.$message({
              message: "删除成功",
