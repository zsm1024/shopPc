  ...
  <div class="table">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-lx-people"></i> 服务费管理
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
    ...
    >
      <el-form ref="role" :model="role" label-width="100px" style="height:300px;overflow-y:auto" :rules="rules">
        <el-form-item label="包装类型" prop="type">
            <el-select v-model="role.type" placeholder="">
                <el-option
                    label="包"
                    value="包">
                </el-option>
          ...
          <!-- <el-input v-model.trim="role.type" maxlength="10"></el-input> -->
        </el-form-item>
        <el-form-item label="下限" prop="lowerCount">
          <el-input v-model.trim="role.lowerCount" maxlength="10"></el-input>
        </el-form-item>
        <el-form-item label="上限" prop="upperCount">
          <el-input v-model.trim="role.upperCount" maxlength="10"></el-input>
        </el-form-item>
        <el-form-item label="单位费用" prop="fee">
          <el-input v-model.trim="role.fee" maxlength="10"></el-input>
        </el-form-item>     
      </el-form>
...
export default {
  ...
  data() {
    return {
      tableColum: [
        { title: "包装类型", prop: "type" },
        { title: "下限", prop: "lowerCount" },
        { title: "上限", prop: "upperCount" },
        { title: "单位费用", prop: "fee" }
      ],
  ...
  methods: {
    ...
    getData() {
      ...
      RoleApi.serviceFeelist(this.req).then(
        res => {
          this.loading = false;
          if (!res.data.error) {
    ...
    //   this.loading = true;
      console.log("row", row);
    //   RoleApi.getinfo({ id: row.id }).then(
    //     res => {
    //       this.loading = false;
    //       if (res.error === false) {
            this.editVisible = true;
    ...
    saveEdit(role) {
      ...
      this.$refs[role].validate(valid => {
        if (valid) {
          this.loading = true;
          RoleApi.serviceFeesave(this.role).then(
            res => {
              this.loading = false;
              if (!res.data.error) {
                this.editVisible = false;
    ...
    // 确定删除
    deleteRow() {
      RoleApi.servicedelete(this.ids).then(
        res => {
          if (!res.data.error) {
               this.$message({
                    message:"删除成功",
